...
RUN set -x \
    ...
    && ./configure --build=x86_64-linux-gnu --host=aarch64-linux-android --prefix=${PREFIX} --disable-rpath --enable-static --disable-shared \
...
RUN set -x \
    ...
    && ANDROID_NDK_ROOT=${ANDROID_NDK_ROOT} ./Configure android-arm64 \
...
RUN set -x \
    ...
    && ./autogen.sh \
    && ./configure --prefix=${PREFIX} --host=aarch64-linux-android --enable-static --disable-shared --without-libsodium --disable-curve --without-documentation \
    && make -j${NPROC} \
    && make install
...
RUN set -x \
    ...
    && ./autogen.sh \
    && ./configure --prefix=${PREFIX} --host=aarch64-linux-android --enable-static --disable-shared --with-pic \
    && make -j${NPROC} \
    && make install
